...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join(os.path.dirname(cfg.__file__), 'project1')
        ...
        for line in file:
            ...
            if line:
                ...
                if len(parts) == 2:
                    ...
                    exchange = ''.join(char for char in exchange if char.isalpha()).lower()
                    ticker = ''.join(char for char in ticker if char.isalpha()).lower()
                    if exchange and ticker:
                        ticker_dict[ticker] = exchange
...
def read_dat(tic):
    ...
    # Create the file path using DATDIR and tic
...
def line_to_dict(line):
    ...
    # Iterate through the columns in the order defined in COLUMNS
    for col in COLUMNS:
        width = COLWIDTHS[col]
    ...
    if tickers_lst is not None:
        ...
        for ticker in tickers_lst:
            if ticker.lower() not in tic_exchange_dic:
                raise Exception(f"Ticker '{ticker}' does not correspond to a valid key in the tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        # Rule 1
        if not col_lst:
            raise Exception("Column names list is empty.")

        # Rule 2
        for col in col_lst:
            if col not in COLUMNS:
                raise Exception(f"Column name '{col}' is not found in COLUMNS.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        for line in lines:
            processed_data = line_to_dict(line)
            filtered_data = {col: processed_data[col] for col in col_lst}
