...
def read_prc_csv(tic):
    ...
    """
    # <COMPLETE THIS PART>
    # Ensure the ticker is in uppercase since TICMAP keys are in uppercase
    tic = tic.upper()

    # Check if the ticker is valid
    if tic not in cfg.TICKERS:
        raise ValueError(f"Ticker {tic} is not recognized. Please use a valid ticker.")

    # Construct the file path for the CSV
    file_path = os.path.join(cfg.DATADIR, tic.lower() + '_prc.csv')

    # Read the CSV file into a Pandas DataFrame
    df = pd.read_csv(file_path)

    # Convert the 'Date' column to datetime objects and set as the index
    df['Date'] = pd.to_datetime(df['Date'])
    df.set_index('Date', inplace=True)

    # Use the standardise_colnames function to rename the columns
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    """
    # <COMPLETE THIS PART>
    # Initialize an empty DataFrame to hold all ticker data
    combined_df = pd.DataFrame()

    # Loop through each ticker
    for ticker in tickers:
        # Read the price data for the ticker
        df = read_prc_csv(ticker)

        # Check if the prc_col exists in the data
        if prc_col not in df.columns:
            raise ValueError(f"The column {prc_col} does not exist in the data for ticker {ticker}.")

        # Extract the relevant price column and rename it to the ticker's name
        df_ticker = df[[prc_col]].rename(columns={prc_col: ticker.lower()})

        # Combine the ticker's data with the overall DataFrame
        if combined_df.empty:
            combined_df = df_ticker
        else:
            combined_df = combined_df.join(df_ticker, how='outer')
...
def mk_ret_df(prc_df):
    ...
    mkt_df = pd.read_csv(cfg.FF_CSV, parse_dates=['Date'])
    mkt_df.set_index('Date', inplace=True)
    mkt_df = mkt_df[mkt_df.index.isin(prc_df.index)]
    ret_df['mkt'] = mkt_df['mkt']
    ret_df.iloc[0, :-1] = pd.NA
...
def mk_aret_df(ret_df):
    ...
    for col in aret_df.columns:
        if col != 'mkt':  # Skip the market column
...
def mk_aret_df(ret_df):
    ...
    """
    # <COMPLETE THIS PART>
    stock_returns = ret_df.drop(columns=['mkt'])
    aret_df = stock_returns.sub(ret_df['mkt'], axis=0)
    return aret_df
...
def get_avg(df, col, year):
    ...
    """
    #<COMPLETE THIS PART>
    # Filter the DataFrame for the year and specific column
    year_df = df.loc[df.index.year == year, col]
    # Calculate and return the mean, excluding NaN values
    return year_df.mean()
...
def get_ew_rets(df, tickers):
    ...
    """
    #<COMPLETE THIS PART>
    # Select the specified tickers and calculate the mean along the columns
    ew_rets = df[tickers].mean(axis=1, skipna=True)
    return ew_rets
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    # Filter the series for the specified period
    ret_ser = ser[start:end]

    tot_ret = (1 + ret_ser).prod()
